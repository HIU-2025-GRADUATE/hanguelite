...
class Vdbe:
  ...
  ** 이 로직은 정확히 생성자가 하는 일과 일치해서 해당 함수는 제거하고 생성자로 대체함.
  """
  def __init__(self, pBe: Dbbe = None):
    # DB backend 객체
    # self.pBe = 0
  ...
  def addOp(self, op: int=0, p1: int=0, p2: int=0, p3: str=0, lbl: int=0) -> int:
  ...
  def exec(self, xCallback=0, pArg=0, pzErrMsg: str=0, pBusyArg=0, xBusy=0) -> int:
    ...
    while pc < self.nOp:
      ...
      # 스택에서 원소 p1개를 pop
        ...
        self.aStack = self.aStack[-pOp.p1]
      ...
      # 굳이 그렇게할 필요 없을 것 같아서 일단 리스트 하나로 묶어서 Record로 만듬
